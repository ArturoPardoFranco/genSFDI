...
'''
Convolutional Info-VAE (a.k.a. MMD-VAE) with deterministic bottleneck and skip connections
'''
class ConvolutionalInfoSCVAE:
    ...
    def prepare_graph(self):
        ...
        #self.gce_connected.append(self.X)
            ...
            with tf.name_scope('conv_encoder_l' + str(i)):
                ...
                for j in range(0, i+1):
                    ...
                    if j == 0:
                        ...
                        nmw = np.sqrt(2.0/(int(self.gce_connected[i-1].shape[3])*self.fs[i-1]*self.fs[i-1]))
                        self.Wce.append(tf.Variable(nmw*self.init([self.fs[i-1], self.fs[i-1], int(self.gce_connected[i-1].shape[3]), self.n_conv[i-1]], dtype=tf.float32), name='Wce' + str(i)))
        ...
        with tf.name_scope("z"):
            ...
            for j in range(0, i + 1):
                ...
                if j == 0:
                    ...
                    # Create weights
                    self.w_z.append(
                        tf.Variable(nmw*self.init([self.n_z, int(self.fe.shape[0])]), name='We_' + str(j) + '_' + str(i)))
                    #self.g_z.append(tf.matmul(self.w_z[j], self.fe))
            ...
            self.b_z = tf.Variable(tf.zeros([self.n_z, 1]), name='b_mu')
            self.z = tf.add(tf.add_n(self.g_z), self.b_z, name='mu')
        ...
        with tf.name_scope("glob_avg_fd"):
            ...
            for j in range(0, i + 1):
                ...
                if j == 0:
                    nmw = np.sqrt(2.0/self.n_z)
                    # Create weights
                    self.w_fd.append(tf.Variable(nmw*self.init([int(self.fe.shape[0]), self.n_z]), name='W_' + str(j) + '_Xhat'))
            ...
            self.fd = tf.nn.dropout(self.hidden_activation(tf.add(tf.add_n(self.g_fd), self.b_fd), name='fd'), keep_prob=self.dropout_rate)
            ...
            self.fd_rescaled = tf.image.resize_images(tf.expand_dims(tf.expand_dims(tf.transpose(self.fd), 1), 1),
                                                      size=(int(self.gce_connected[-1].shape[1]), int(self.gce_connected[-1].shape[2])))
            ...
            with tf.name_scope('conv_decoder_l' + str(i)):
                ...
                for j in range(0, i + 1):
                    ...
                    if j == 0:
                        ...
                        self.Wcd.append(tf.Variable(nmw*self.init([self.fs_d[i - 1], self.fs_d[i - 1], int(self.gcd_connected[i-1].shape[3]), self.n_conv_d[i - 1]],
                            dtype=tf.float32), name='Wcd' + str(i)))
                        self.bcd.append(tf.Variable(tf.zeros([self.n_conv_d[i - 1]], dtype=tf.float32), name='bcd' + str(i)))


                        g_curr.append(tf.nn.dropout(self.hidden_activation(tf.nn.bias_add(
                            tf.nn.conv2d(tf.image.resize_images(self.gcd_connected[i-1], size=(int(self.gcd_connected[i-1].shape[1])*self.nstride[i-1], int(self.gcd_connected[i-1].shape[2])*self.nstride[i-1])),
        ...
        with tf.name_scope('Xhat'):
            ...
            self.W_Xhat = tf.Variable(nmw*self.init([self.fs_d[-1], self.fs_d[-1], int(self.gcd_connected[-1].shape[3]), self.channels],
                            dtype=tf.float32), name='W_Xhat' + str(i))
            self.b_Xhat = tf.Variable(tf.zeros([self.channels], dtype=tf.float32), name='bcd' + str(i))
            self.Xhat = tf.nn.bias_add(tf.nn.conv2d(tf.image.resize_images(self.gcd_connected[-1], size=(self.width, self.height)),
...
if __name__ == '__main__':
    ...
    network = ConvolutionalInfoSCVAE(nc, fs, nstride, nh, nz, x_dims, fresh_init=1, mbs=16, cost_function='MSE',
                                     actype='lrelu', name='conv_SCVAE_tests', optim='Adam', reset_default_graph=1)
